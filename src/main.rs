...
fn main() {
    ...
    let unlock_spec: Schedule = cfg
        .unlock_on
        .parse()
        .expect("Invalid unlock_on specification");
    let lock_spec: Schedule = cfg.lock_on.parse().expect("Invalid lock_on specification");
    let should_be_locked =
        lock_spec.upcoming(offset::Utc).next() > unlock_spec.upcoming(offset::Utc).next(); //bad idea

    println!("next lock: {:?}", lock_spec.upcoming(offset::Utc).next());
    println!(
        "next unlock: {:?}",
        unlock_spec.upcoming(offset::Utc).next()
    );

    println!("starting");
    let mut client =
        Client::new(cfg.discord_token.clone(), Handler).expect("Error creating client");
    ...
    let scheduler_thread = thread::spawn(move || {
        ...
        let job_fn = |msg: &str, perm: PartialPermissionOverwrite, locked: bool| {
            ...
            (*state_guard).guilds.iter().for_each(|p| {
                let gctx = GuildContext {
                    ctx: &*p.0,
                    guild: &p.1,
                };
                gctx.change_channel_permissions(
                    cfg.channel_name.as_str(),
                    cfg.role_name.as_str(),
                    perm.clone(),
                )
                .map_err(|err| println!("failed to change perms of the channel {:?}", err))
                .ok();
                if let Ok(Some(ch)) = gctx.find_channel(cfg.channel_name.as_str()) {
                    ch.send_message(&*p.0.http.clone(), |m| m.content(msg))
                        .map_err(|err| println!("failed the send a message {:?}", err))
                        .ok();
                }
            });
        };
        ...
        scheduler.add(Job::new(unlock_spec, || {
            println!("unlocking");
            job_fn(
                cfg.unlock_message.as_str(),
                create_unlock_permisson(),
                false,
            );
            println!("done");
        }));
    ...
    client.with_framework(StandardFramework::new().configure(|c| c.prefix("~")).after(
        |_, _, _, err| {
            if err.is_err() {
                println!("error: {:?}", err);
            }
    ...
    scheduler_thread
        .join()
        .map_err(|err| println!("failed to join the scheduler thread {:?}", err))
        .unwrap();
}
